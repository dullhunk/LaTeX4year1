...
\hypertarget{written-communciation-with-latex}{%
\chapter{Written communciation with LaTeX}\label{written-communciation-with-latex}}
...
\item
  Extend your knowledge of LaTeX and know where to find out more
\end{enumerate}
...
To turn this into a PDF we need to use a LaTeX compiler, we're going to use pdflatex, though several other compeilers are available.\footnote{\url{https://www.overleaf.com/learn/latex/Choosing_a_LaTeX_Compiler}} The pdflatex compiler is already installed on the Linux VM image for VirtualBox.\footnote{\url{https://wiki.cs.manchester.ac.uk/index.php/CSImage_VM}} For other operating systems you'll need to explore the options at \href{https://www.latex-project.org/get/}{latex-project.org/get}.
...
The text here is excerpted from \emph{The New Turing Ominbus: 66 excursions in Computer Science} \citep{turingomnibus}.

\hypertarget{exercise-one-document}{%
\section{Exercise one: document}\label{exercise-one-document}}

Cut-and-paste this longer text into your document or make your own sections and text. You could use text from \href{https://en.wikipedia.org/wiki/Lorem_ipsum}{Lorem ipsum} at \href{https://www.lipsum.com}{lipsum.com} to fill out the page.

Now, at the top of your document after the \textbackslash begin\{document\} line and before the command defining the first section, add the following commands, each on their own line:
...
The \texttt{title}, \texttt{author}, \texttt{tableofcontents} and \texttt{newpage} commands are self-explanatory. The \texttt{maketitle} automatically generates a date Your table of contents won't be created until you run pdflatex \textbf{twice}. You need to process the source twice because on the first run, LaTeX gathers and stores information about what to put in the table of contents, and only creates it on the second run.
...
\tightlist
\item
  Notice how bold and italic formatting are created using textbf and textit
\item
  Notice how lists are created using enumerate and item
\item
  Notice that curved open quotation (`` - looks like 66) and close quotation marks ('' - looks like 99) are different characters. Look carefully at the quotes around the phrase ``Can machines think?'' in the excerpt above in section \protect\hyperlink{longer}{longer}.
\end{itemize}
...
A document without figures, images, picture and graphs is pretty dull, so you'll want to illustrate your document with figures such as the one in Figure \ref{fig:turing-fig}, for example.
...
Footnotes\footnote{Like this one which is a footnote about footnotes. Very meta.} can be added to a document with \texttt{\textbackslash{}footnote\{\}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{\textbackslash{}footnote}\NormalTok{\{This is a footnote\}}
\end{Highlighting}
\end{Shaded}
...
The citations themselves can be stored in a separate \texttt{*.bib\ file} using a format called BibTeX. BibTeX gives provides citation types so books\footnote{Using tools like \url{https://www.ottobib.com} can help you to quickly generate BibTeX entries from a given ISBN number} look like this:
...
and articles\footnote{BibTeX entries for journal articles can be automatically generated from unique identifiers known as digital object identifiers, see \url{https://doi2bib.org} for example. This saves you the hassle of typing them in by hand.} look like this:
...
For web references you can use misc
...
``You can find Alan Turing's publications in Google scholar. \citep{googlescholar}
This includes his paper on the Entscheidungsproblem published in 1937. \citep{turing}''

We would need the following in our tex file:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{You can find Alan Turing\textquotesingle{}s publications in Google scholar. }\KeywordTok{\textbackslash{}cite}\NormalTok{\{}\ExtensionTok{googlescholar}\NormalTok{\}}
\NormalTok{This includes his paper on the Entscheidungsproblem published in 1937. }\KeywordTok{\textbackslash{}cite}\NormalTok{\{}\ExtensionTok{turing}\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{maths}{%
\chapter{Doing the math}\label{maths}}
...
\hypertarget{exercise-3-maths}{%
\section{Exercise 3: maths}\label{exercise-3-maths}}
...
We've briefly looked at typesetting mathematics in LaTeX, in the next chapter we'll look at collaborative authoring with overleaf.
...
\caption{A screenshot of overleaf showing the source TeX on the left and side and the corresponding pdf on the right hand side}\label{fig:overleaf-fig}
\end{figure}
...
Overleaf exercise:
1. Login to \href{https://www.overleaf.com/}{overleaf.com}
2. Create and save a simple document using the tutorial at \href{https://www.overleaf.com/learn/how-to/Creating_a_document_in_Overleaf}{overleaf.com/learn/how-to/Creating\_a\_document\_in\_Overleaf}
3. Note that you could store your TeX source in a git repository if you wanted to
...
You'll write many documents at University, but there is one document that is \textbf{really} seriously important in a potentially life-changing way. Your curriculum vitae\footnote{or curriculum vitæ (not vitae) if you're being pedantic} or resume. An example CV is shown in Figure \ref{fig:cv-fig}.
...
\caption{An example CV using an overleaf template. Would you invite Jake to interview based on his CV? There are many other templates available, this one is by Jake Gutierrez and published under an MIT license at [overleaf.com/latex/templates/jakes-resume/syzfjbzwjncs](https://www.overleaf.com/latex/templates/jakes-resume/syzfjbzwjncs)}\label{fig:cv-fig}
\end{figure}

Creating a professional document is especially important when you write your CV, because it determines if you are invited to interview for opportunities you are applying for. The decision to interview is typically based on:

\begin{itemize}
\tightlist
\item
  how your CV looks, it's typesetting, layout and style
\item
  the content of your CV, what you've done and how you describe it
\item
  the quality and clarity of your written communication
\item
  the editing, what you've decided to leave in and out of your CV
\end{itemize}

By opportunities we mean immediate ones within the next 12 months as well as those further in the future. Those opportunities might include:

\begin{itemize}
\tightlist
\item
  Spring insights during easter next year\footnote{see e.g.~\url{https://www.ratemyplacement.co.uk/insights}}
\item
...
So it is well worth investing a bit of your time to create version 1.0 of your CV. It's never too early to make a start.

\hypertarget{exercise-five-your-cv}{%
\section{Exercise five: your CV}\label{exercise-five-your-cv}}

Create a basic CV which describes your story including:

\begin{itemize}
\tightlist
\item
  your education, including University
\item
  your experience, voluntary, paid or otherwise
\item
  your projects: personal, technical and non-technical
\end{itemize}

You can do this using the ready-made templates at \url{https://www.overleaf.com/gallery/tagged/cv}

\hypertarget{debug-your-cv}{%
\section{Debug your CV}\label{debug-your-cv}}

As you progress through University, it is a good idea to keep your CV up to date, and get feedback from as many people as you can. There will be opportunities to debug your CV later, but you'll need a ``beta release'' (version 1.0) of your CV to get started. The best time to start debugging is now, so that you can squash any bugs before employers see them. Here's some common bugs we've seen in CVs.
...
\item
  Does it fit comfortably on preferably one (Résumé) or two pages (CV) only?
\item
  Is it in reverse chronological order? Most important (usually recent) things first?
\item
  Have you talked about what you have actually done using prominent verbs, rather than just what you think you know? See \href{http://git.io/verbsfirst}{git.io/verbsfirst} for examples
\item
  Have you mentioned things you are studying now and throughout the year, not just courses you have finished?
\item
  Have you quantified and provided evidence for the claims you make?
\item
  Is it robot proof? Feed it through software like \href{http://careerset.io/manchester}{careerset.io/manchester} and \href{https://resume.io/}{resume.io}. What feedback do the robots give you?
\item
  Find out more in the \emph{Debug your CV} guide at \href{http://git.io/mycv}{git.io/mycv}
\end{enumerate}

\hypertarget{conclusion}{%
\section{Conclusion}\label{conclusion}}

Your CV is a \emph{really} important document. We recommend you start working on it sooner rather than later and get feedback from as many different people as you reasonably can.
