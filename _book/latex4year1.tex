...
\item
  Draft a CV using LaTeX templates in overleaf
\end{enumerate}
...
LaTeX is a document preparation system for high-quality typesetting. \citep{latexproject} LaTeX is a mature and established typesetting system that has been around since 1980's. \citep{knuth} It was originally written by \href{https://en.wikipedia.org/wiki/Leslie_Lamport}{Leslie Lamport}, a computer scientist
now working at Microsoft Research, although its development has long been taken over by a world-wide LaTeX community. It's actually built on top of another system called TeX, a computer typesetting system designed by another influential Computer Scientist, \href{https://en.wikipedia.org/wiki/Donald_Knuth}{Donald Knuth} of Stanford University. Lamport and Knuth are shown in Figure \ref{fig:knuthport-fig}.

\begin{figure}
\includegraphics[width=0.47\linewidth]{images/Leslie_Lamport} \includegraphics[width=0.47\linewidth]{images/KnuthAtOpenContentAlliance} \caption{Turing award winners Leslie Lamport and Donald Knuth created TeX and LaTeX during the 1980's. Lamport portrait by Lamport, GFDL via Wikimedia Commons [w.wiki/oyM](https://w.wiki/oyM), Knuth portrait by Jacob Appelbaum CC BY-SA 2.5 via Wikimedia Commons [w.wiki/oyL](https://w.wiki/oyL)}\label{fig:knuthport-fig}
\end{figure}
...
LaTeX is \textbf{not} a WYSIWYG system (what you see is what you get) either. The raw document you edit (a \texttt{*.tex} file) is not your final output (usually a \texttt{*.pdf} file).
...
To generate the bibliography you'll need to specify what style of bibliography you're using with \texttt{\textbackslash{}bibliographystyle\{stylename\}} and \texttt{\textbackslash{}bibliography\{bibfile\}}. The example below uses \texttt{unsrt} for style and references a bib file called \texttt{turing.bib} which might contain many articles and books. You don't need to specific the file extension \texttt{.bib}:
...
\item
  your \emph{projects}, both personal or at University and beyond
\end{itemize}
