...
## The shortest document possible {#shortest}
...
The text here is excerpted from *The New Turing Ominbus: 66 excursions in Computer Science* [@turingomnibus]. The Omnibus is a lovely introduction to the fundamentals of Computer Science that you might enjoy. The software engineer [Jeff Atwood](https://en.wikipedia.org/wiki/Jeff_Atwood) in his book review called it an “incredibly fun little book”. [@codinghorror]

## Exercise one: document
Cut-and-paste this longer text into your document or make your own sections and text. You could use text from [Lorem ipsum](https://en.wikipedia.org/wiki/Lorem_ipsum) at [lipsum.com](https://www.lipsum.com) to fill out the page.

Now, at the top of your document after the \begin{document} line and before the command defining the first section, add the following commands, each on their own line:
...
The `title`, `author`, `tableofcontents` and `newpage` commands are self-explanatory. The `maketitle` automatically generates a date Your table of contents won't be created until you run pdflatex **twice**. You need to process the source twice because on the first run, LaTeX gathers and stores information about what to put in the table of contents, and only creates it on the second run.
...
* Notice that curved open quotation (“ - looks like 66) and close quotation marks (” - looks like 99) are different characters. Look carefully at the quotes around the phrase “Can machines think?” in the excerpt above in section [longer](#longer).
