...
Footnotes^[Like this one which is a footnote about footnotes. Very meta.] can be added to a document with `\footnote{}`
...
\cite{turing}
```
The citations themselves can be stored in a separate `*.bib file` using a format called BibTeX. BibTeX gives provides citation types so books^[Using tools like https://www.ottobib.com can help you to quickly generate BibTeX entries from a given ISBN number] look like this:
...
```
and articles^[BibTeX entries for journal articles can be automatically generated from unique identifiers known as digital object identifiers, see https://doi2bib.org for example. This saves you the hassle of typing them in by hand.] look like this:
...
“You can find Alan Turing's publications in Google scholar. [@googlescholar]
This includes his paper on the Entscheidungsproblem published in 1937. [@turing]”

We would need the following in our tex file:

```tex
You can find Alan Turing's publications in Google scholar. \cite{googlescholar}
This includes his paper on the Entscheidungsproblem published in 1937. \cite{turing}
```
