...
To generate the bibliography you'll need to specify what style of bibliography you're using with `\bibliographystyle{stylename}` and `\bibliography{bibfile}`. The example below uses `unsrt` for style and references a bib file called `turing.bib` which might contain many articles and books. You don't need to specific the file extension `.bib`:
